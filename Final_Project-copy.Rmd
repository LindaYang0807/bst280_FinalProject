...
#### 5. EDA: PCA and Clustering across All Genes
Principal component analysis (PCA) was applied to the vst-normalized expression matrix to visualize global transcriptional differences between ER groups. Samples were projected onto the first two principal components, and ER status was overlaid as a grouping variable.

Unsupervised clustering was conducted using both k-means clustering (k = 2) and hierarchical clustering (Ward.D2 linkage). Contingency tables comparing cluster assignments versus ER labels were generated to assess natural separation between ER subtypes. Hierarchical clustering dendrograms were also exported for visualization.
...
A preranked GSEA was performed using fgsea, applying DESeq2 Wald statistics as the ranking metric. Gene-level statistics were ordered decreasingly and passed to GO Biological Process (<GO:BP>) gene sets retrieved through msigdbr.

fgsea was run using 2000 permutations to evaluate pathway-level enrichment. Adjusted p-values were computed, and results were sorted and exported. This analysis identifies biological processes disproportionately activated in ERpos or ERneg tumors.
